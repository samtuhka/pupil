...
def eye(g_pool,cap_src,cap_size,rx_from_world,eye_id=0):
    ...
    #flushing pupil queue in case world process did not exit gracefully
    while not g_pool.pupil_queue.empty():
        g_pool.pupil_queue.get()
    g_pool.pupil_queue.close()

    #flushing glint queue in case world process did not exit gracefully
    while not g_pool.glints.empty():
        g_pool.glints.get()
    g_pool.glints.close()

    #flushing glint-pupil vector queue in case world process did not exit gracefully
    while not g_pool.glint_pupil_vectors.empty():
        g_pool.glint_pupil_vectors.get()
    g_pool.glint_pupil_vectors.close()
